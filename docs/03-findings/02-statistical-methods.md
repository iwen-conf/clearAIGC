...
> 本文档是 `01-claude-ai-rate-solution.md` 的技术附篇，用来沉淀“有哪些统计量可用于构造 AI 风险评分和改写约束”。
>
> 2026-04-20 注：本文是**方法库**，不是当前仓库的实现清单。文中的 `perplexity.go` / `burstiness.go` / `ngram.go` 等代码块主要用于说明算法结构，不能直接视为当前代码文件名。当前实际落地文件请以 `01-claude-ai-rate-solution.md` 和仓库中的 `internal/domain/scorer/*` 为准。
>
> 核心观点仍成立：**主流 AI 检测器的判别逻辑，几乎都可以拆解为公开、可推导的统计量。只要围绕这些统计量进行反向优化，“降低 AI 率”就能从经验操作转化为可验证的数学问题。**
...
从工程角度看，这张表给出的并不是“检测器名单”，而是一组可量化的判别信号。把这些判别量反向转换为约束条件，就是降低 AI 率的目标函数来源。
...
每个指标都按同一结构展开：**数学定义 / AI 与人类基线 / 优化方向 / Go 实现草图 / Prompt 注入模板**。
...
**优点**：无需额外训练，兼容任何闭源 API（GPT-4 / Claude / Gemini）。
**缺点**：单次生成通常难以同时满足所有约束，因此需要 2~3 轮反馈。
...
**优点**：约束直接作用在解码阶段，理论上更稳定。
**缺点**：前提是系统必须拿到 logits 访问权。

**适用场景**：Naturalize 可以把 Qwen2.5 / DeepSeek 等开源模型作为**内部 rewriter 路径**启用约束解码；面向 GPT / Claude 等闭源模型时，则采用路径 A。
...
Agent B 是一个**经过校准的内部 scorer**，扮演“检测器”角色，逐句给分并把信号反馈给 Agent A。Agent A 再按反馈重写，直到达到阈值或触及上限。

**与 Naturalize 现有架构的关系**：现有 coordinator 已经具备 ReAct 结构，因此 **Detector 可以作为一个新的 tool 注入**：
...
这份模板可以视为 **“让 AI 遵循统计学方案”** 的最终形态，可直接写入 `prompts/sentence_rewriter_cn.md`：
...
**关键点**：每一条指令背后都对应一个数学目标和后置校验。系统不是要求模型“尽量写好”，而是要求它**命中可验证的数值目标**。
...
最后一项是 Style Anchor 路径的核心指标：**带锚点的用户，其降分效果应比未带锚点的用户至少高出 1.5 倍**。
...
这些参考文献不是装饰性的背景材料，而是每个指标背后的第一性依据。正式落地前，至少应通读摘要并确认指标边界。
...
**降低 AI 率并不是经验主义操作，而是一组可量化、可验证的数学约束。** 主流检测器的判别逻辑大体可拆解、可测量，因此也可以被系统性地逆向约束。

本文档给出的 12 个指标、3 条落地路径和 1 份主 prompt 模板，可以把 Naturalize 从“让模型多润色几遍”的无目标改写模式，升级为**“带数学目标的闭环优化系统”**。

其中最值得持续投入的是 **Style Anchor**。一旦系统能稳定对齐到用户本人的历史写作分布，通用检测器的判别基础就会被显著削弱，这也是产品最可持续的差异化能力。
