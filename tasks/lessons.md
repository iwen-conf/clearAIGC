...
- If a user reports a workflow that looks stuck mid-progress, compare the live `/state` payload against the SSE connection lifetime before touching business logic; an HTTP `WriteTimeout` can silently kill stream updates while the backend keeps processing normally.
