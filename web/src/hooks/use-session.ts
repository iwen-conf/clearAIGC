...
export function useSession(): UseSessionResult {
  ...
  const totalPasses = session?.totalRounds ?? 0;
  const nextRoundNumber =
    session?.nextRound && session.nextRound > 0 ? session.nextRound : null;
  const canStartNextRound = Boolean(session?.canStartNextRound);
  ...
  useEffect(() => {
    ...
    const source = new EventSource(
      `/api/v1/sessions/${activeSessionId}/stream`,
    );
  ...
  const acceptCard = useCallback(
    (cardId: string) => mutateCardState(cardId, "accepted"),
    [mutateCardState],
  );
  const rejectCard = useCallback(
    (cardId: string) => mutateCardState(cardId, "rejected"),
    [mutateCardState],
  );
  ...
  const applyAll = useCallback(async () => {
    ...
    setError(null);
    try {
      const { updated } = await apiApplyAllCards(
        session.id,
